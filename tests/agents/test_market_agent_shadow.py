...
from agents.market_agent import check_circuit_breakers
...
class TestCheckCircuitBreakers:
    ...
    def test_coordinated_selloff_needs_all_three(self):
        # Only SP and NQ down > 1%, Dow flat — should NOT trigger
    ...
    def test_vix_checked_before_futures(self):
        # Both conditions true — VIX fires first
...
def _make_shadow_response(json_str: str):
    return make_api_response("end_turn", [text_block(json_str)])


class TestRunMarketAgentShadow:
    def _params(self):
    ...
    def _tracer(self, mock_supabase):
        ...
        return TraceLogger("test-shadow-session")

    def test_returns_skip_when_circuit_breaker_fires(self, mock_supabase):
        tracer = self._tracer(mock_supabase)
        mock_supabase.table.return_value = make_query([])
        with patch("agents.market_agent_shadow.get_vix",     return_value=_CB_VIX), \
             patch("agents.market_agent_shadow.get_futures", return_value=_CB_FUTURES):
            result = run_market_agent_shadow(tracer, self._params())
    ...
    def test_cb_skip_does_not_call_claude(self, mock_supabase):
        ...
        with patch("agents.market_agent_shadow.get_vix",     return_value=_CB_VIX), \
            ...
            run_market_agent_shadow(tracer, self._params())
    ...
    def test_calls_claude_when_no_circuit_breaker(self, mock_supabase):
        ...
        with patch("agents.market_agent_shadow.get_vix",     return_value=_NORMAL_VIX), \
             patch("agents.market_agent_shadow.get_futures", return_value=_NORMAL_FUTURES), \
             patch("anthropic.Anthropic") as mock_client_cls:
            mock_client = MagicMock()
            mock_client_cls.return_value = mock_client
            mock_client.messages.create.side_effect = [tool_resp, final]
            with patch("agents.market_agent_shadow.get_fear_greed",        return_value={"value": 60}), \
                 patch("agents.market_agent_shadow.get_sector_rotation",   return_value=[]), \
                 patch("agents.market_agent_shadow.get_economic_calendar", return_value={"has_high_impact_events": False}), \
                 patch("agents.market_agent_shadow.get_treasury_yields",   return_value={"yield_10y": 4.5, "change_bp": 3, "direction": "flat"}):
                result = run_market_agent_shadow(tracer, self._params())
    ...
    def test_result_has_circuit_breaker_none_on_normal_run(self, mock_supabase):
        ...
        with patch("agents.market_agent_shadow.get_vix",     return_value=_NORMAL_VIX), \
            ...
            mock_client_cls.return_value = mock_client
            mock_client.messages.create.side_effect = [tool_resp, final]
            with patch("agents.market_agent_shadow.get_fear_greed",        return_value={}), \
                 patch("agents.market_agent_shadow.get_sector_rotation",   return_value=[]), \
                 patch("agents.market_agent_shadow.get_economic_calendar", return_value={}), \
                 patch("agents.market_agent_shadow.get_treasury_yields",   return_value={}):
                result = run_market_agent_shadow(tracer, self._params())
