...
def run_market_agent_shadow(tracer: TraceLogger, params: StrategyParams) -> dict:
    """
    Shadow (autonomous) market agent. Runs alongside the baseline V1 agent for
    comparison — its decision does NOT affect trade execution.
    ...
    if triggered:
        tracer.log_decision("market_shadow", "skip", detail={"circuit_breaker": reason})
        return _cb_skip(reason)

    tracer.start_agent_span("market_shadow")
